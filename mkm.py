...
class MKM:
    ...
    of the global reaction, like reversibility and degree of rate control.
    
    Attributes: 
        name(string): Name of the system under study
        rm_input_file(string): Path to the .mkm file containing the reaction mechanism.
        g_input_file(string): Path to the .mk mfile containing the Gibbs energy for TS, surface
                              and gas species.   
        reactor_model(string): reactor model used for virtual testing. Two available options:
...
#-------------------------------------------------------------------------------------------------------------#
    def __init__(self,
        ...
        for reaction in range(self.NGR):
            ...
            global_reaction_string.append(" ".join(lines[reaction].split()[2:]))
        ...
        for i in range(self.NR):
            ...
            self.dg_reaction[i] = self.dh_reaction[i] - t_ref * self.ds_reaction[i]
            condition1 = self.g_ts[i] != 0.0
            ind = list(np.where(self.v_matrix[:,i] == -1)[0]) + list(np.where(self.v_matrix[:,i] == -2)[0])
            ...
            if condition1 and condition2: # Activated elementary reaction
                ...
                self.ds_barrier_rev[i] = self.ds_barrier[i] - self.ds_reaction[i]
                self.dg_barrier[i] = self.g_ts[i] - gis
                self.dg_barrier_rev[i] = self.dg_barrier[i] - self.dg_reaction[i]
...
#-------------------------------------------------------------------------------------------------------------#
    ...
    def CSTR_params(self,
        ...
        self.CSTR_V = (3.14159265 * radius**2) * length  
        self.CSTR_Q = Q 
    ...
    def apply_ts_scaling(self,
        ...
        if initial_state:
            ...
            ind = list(np.where(self.v_matrix[:,i] == 1)[0]) + list(np.where(self.v_matrix[:,i] == 2)[0])
            hfs = sum([self.h_species[j]*self.v_matrix[j,i]*(-1) for j in ind])
    ...
    def thermochemistry(self, temperature):
        ...
        for global_reaction in range(self.NGR):
            ...
            print("Model:    DHR={:0.2e} kJ/mol    DSR={:0.2e} kJ/mol/K     DGR={:0.2e} kJ/mol".format(DHR_model[global_reaction], DSR_model[global_reaction], DGR_model[global_reaction]))
            print("Database: DHR={:0.2e} kJ/mol    DSR={:0.2e} kJ/mol/K     DGR={:0.2e} kJ/mol".format(DHR_database[global_reaction], DSR_database[global_reaction], DGR_database[global_reaction]))
            print("---------------------------------------------------------------------")
            print("")
        return ""        
    ...
    def thermodynamic_consistency_analysis(self, temperature):
        ...
        extracted by the defined microkinetic model.        
        Args:
            temperature(float): temperature at which the entropic contributions 
        ...
        for global_reaction in range(self.NGR):
            print(self.gr_string[global_reaction])            
            print("Model:    keqH={:0.2e}    keqS={:0.2e}    Keq={:0.2e}".format(keq_H_model[global_reaction], keq_S_model[global_reaction], keq_model[global_reaction]))
    ...
    def kincos_calc(self, temperature, area_active_site=1e-19):
        ...
        """           
        Keq = np.zeros(len(self.reaction_type)) # Equilibrium constant
    ...
    def rate_calc(self, y, kdir, krev, index=None):
        """
        Returns net reaction rate for each elementary reaction step.        
        Args:
            y(nparray): surface coverage fractions vector
            kdir,krev(nparray): kinetic constants of the direct/reverse reaction steps.            
        Returns:
    ...
    def single_run(self,
        ...
        if sum(gas_composition) != 1.0:
            raise ValueError('Wrong gas composition: the sum of the provided molar fractions is not equal to 1.')
        for molar_fraction in gas_composition:
            if (molar_fraction < 0.0) or (molar_fraction > 1.0):
                raise ValueError('Wrong gas composition: molar fractions must be between 0 and 1.')            
        if temperature < 0.0:
        ...
        if self.reactor_model == 'dynamic':  
            ...
            for reaction in range(self.NGR):
                try:
                    ...
                    x = 0                
                RR.append(self.CSTR_Q * (P_out[x]- P_in[x]) / (R * temperature))   # [mol/s]       
            s_target_sr = RR[0] / np.sum(RR)
            r_sr = self.rate_calc(results_sr.y[:,-1], *self.kincos_calc(temperature))    
        ...
        if verbose == 0:
            print('')
            print('{} Reaction Rate: {:0.2e} 1/s'.format(self.target_label, r_sr[self.target]))
            print('{} Selectivity: {:.2f}%'.format(self.target_label, s_target_sr*100.0))
            print('Most Abundant Surface Intermediate: {} Coverage: {:.2f}% '.format(key_masi, value_masi*100.0))
        ...
        for i in range(len(temp_vector)):
            for j in range(len(p_vector)):
                run = self.single_run(temp_vector[i],
                                      ...
                                      verbose=0)
                r_matrix[i, j] = list(run['r'].values())[self.grl[global_reaction_label]]                 
        return r_matrix
    
    def __calc_eapp(self, temperature_vector, reaction_rate_vector):
        """
        ...
        reg = lm.fit(x, y)
        Eapp = -R*reg.coef_[0,0] / 1000.0 # kJ/mol
        R2 = reg.score(x, y)
        return Eapp, R2
    ...
    def calc_apparent_activation_energy(self, 
        ...
        """        
        print('{}: Apparent activation energy for {} reaction'.format(self.name, global_reaction_label))
        print('')
        ...
        for i in range(len(temperature_vector)): 
            ...
            print('Temperature = {}K    CPU Time: {:.2f}s'.format(temperature_vector[i],
                                                                  time.time() -t0))
        eapp, r_squared = self.__calc_eapp(np.asarray(temperature_vector), r_ea)    
        keys = ['Tmin', 
                'Tmax',
                'N',
                'P', 
        ...
        plt.title('{}: {} Apparent Activation Energy'.format(self.name, global_reaction_label))
        plt.text(0.65,
                 0.75,
    ...
    def __calc_reac_order(self, partial_pressure, reaction_rate):
        """
        Args:
            partial_pressure(nparray): Partial pressure of the species [Pa]
    ...
    def calc_apparent_reaction_order(self, 
        ...
        for i in composition_matrix[:, index]:
            if i == 0.0:
                raise ValueError('Provide non-zero molar fraction for the gaseous species for which the apparent reaction order is computed.')
        ...
        if switch_ts_int == 0:    # Transition state
            ...
            for i in range(2):
                ...
                for j in range(mk_object.NR):
                    mk_object.dg_reaction[j] = np.sum(mk_object.v_matrix[:,j]*np.array(mk_object.g_species))            
                    condition1 = mk_object.g_ts[j] != 0.0
                    ind = list(np.where(mk_object.v_matrix[:,j] == -1)[0]) + list(np.where(mk_object.v_matrix[:,j] == -2)[0])
                    gis = sum([mk_object.g_species[k]*mk_object.v_matrix[k,j]*(-1) for k in ind])
                    condition2 = mk_object.g_ts[j] > max(gis, gis+mk_object.dg_reaction[j])
            
                    if condition1 and condition2: # Activated elementary reaction
                        mk_object.dg_barrier[j] = mk_object.g_ts[j] - gis
                        mk_object.dg_barrier_rev[j] = mk_object.dg_barrier[j] - mk_object.dg_reaction[j]
                    else:  # Unactivated elementary reaction
    ...
    def calc_drc_full(self,
        ...
        Args:
            ...
            initial_conditions(nparray): Initial surface coverage [-]
            dg(float): Deviation applied to selected Gibbs energy to calculate numerically the DRC/DSC.
                       Default=1E-6 eV                       
        Returns:
    ...
    def __z_calc(self, y, kdir, krev):
        """
        Calculates reversibility of reactions.        
        Args:
        ...
        ri = np.zeros(len(kdir))             
        for i in range(len(rd)):
            rd[i] = kdir[i]*np.prod(y**(self.v_f[:,i])) # Forward reaction rate 
            ri[i] = krev[i]*np.prod(y**(self.v_b[:,i])) # Backward reaction rate    
        return ri/rd                                       
    ...
    def calc_reversibility(self,
        ...
        reversibility = self.__z_calc(composition_ss, *k)
        r = []
        for i in range(self.NR):
            r.append('R{}'.format(i+1))
        ...
        df_reversibility.to_csv("Z_{}_{}_{}.csv".format(self.name, int(temperature), int(pressure/1e5)))
