...
def apply_ts_scaling(reaction, q, m, initial_state=True):
    """
    ...
    i = int(reaction[1:]) - 1
    if initial_state:
        ind = list(np.where(self.v_matrix[:, i] == -1)[0]) + list(
            np.where(self.v_matrix[:, i] == -2)[0]
        )
...
def apply_lsr_1d(
    ...
    if bep:
        ...
        m = scaling_matrix_ts[:, 1]
        for j in range(self.NR):
            self.apply_ts_scaling(
                "R{}".format(j + 1), q[j], m[j], initial_state=initial_state
            )
        self.bep = False
    self.lsr_mode = True
