...
from pymkm.thermo import k_eq_H, k_eq_S, reaction_enthalpy, reaction_entropy
...
class MicrokineticModel:
    ...
    def __init__(self, name: str, rm_input: str, g_input: str, t_ref: float=273.15, inerts: list=[]):
        ...
        self.NGR, self.NR = get_NGR_NR(
            rm_lines
        )  # Number of global (NGR) and elementary (NR) reactions
        self.r = ["R{}".format(i + 1) for i in range(self.NR)]
        self.gr = ["GR{}".format(i + 1) for i in range(self.NGR)]
        global_reaction_label = [
            rm_lines[reaction].split()[0] for reaction in range(self.NGR)
        ]
        global_reaction_index = [
            int(rm_lines[reaction].split()[1][:-1]) for reaction in range(self.NGR)
        ]
        self.gr_string = [
            " ".join(rm_lines[reaction].split()[2:]) for reaction in range(self.NGR)
        ]
        ...
        self.grl = dict(
            zip(global_reaction_label, global_reaction_index)
        )  # ex: "MeOH": 15
        self.gr_dict = dict(
            zip(global_reaction_label, self.gr_string)
        )  # ex: "MeOH": "CO2 + 3H2 -> CH3OH + H2O"
        self.reaction_type = reaction_type(rm_lines, self.NR, self.NGR)
        self.species_sur, self.species_gas, self.species_tot = classify_species(
            get_species_label(rm_lines, self.NGR, inerts)
        )
        self.NC_sur, self.NC_gas, self.NC_tot = get_NC(
            self.species_sur, self.species_gas
        )
        ...
        self.m = ads_mass(
            self.v_matrix, self.reaction_type, self.NC_sur, list(self.MW.values())
        )
        ...
        self.h_species, self.s_species, self.g_species = species_energy(
            preprocess_g(g_input), self.NR, self.NC_tot, t_ref, self.species_tot, inerts
        )
        self.h_ts, self.s_ts, self.g_ts = ts_energy(
            preprocess_g(g_input), self.NR, t_ref
        )
        self.dh_reaction, self.ds_reaction, self.dg_reaction = reaction_energy(
            self.v_matrix, self.h_species, self.s_species, t_ref
        )
        self.dh_barrier, self.dh_barrier_rev = h_barrier(
            self.v_matrix, self.h_ts, self.h_species, self.dh_reaction
        )
        self.ds_barrier, self.ds_barrier_rev = s_barrier(
            self.v_matrix, self.s_ts, self.s_species, self.ds_reaction
        )
        self.dg_barrier, self.dg_barrier_rev = g_barrier(
            self.v_matrix, self.g_ts, self.g_species, self.dg_reaction
        )
        # DataFrames
        self.df_system = pd.DataFrame(
            self.v_matrix, index=self.species_tot, columns=[self.r, self.reaction_type]
        )
        self.df_system.index.name = "species"
        self.df_gibbs = pd.DataFrame(
            np.array([self.dg_reaction, self.dg_barrier, self.dg_barrier_rev]).T,
            index=[self.r, self.reaction_type],
            columns=["DGR / eV", "G_act / eV", "G_act,rev / eV"],
        ).round(2)
    ...
    def set_CSTR_params(
        ...
        if verbose == 0:
            ...
            print("Inlet volumetric flowrate : {:0.2e} [m3/s]".format(self.CSTR_Q))
            print("Residence time: {:0.2e} [s]".format(self.CSTR_tau))
            print("Catalyst mass: {:0.2e} [kg]".format(self.CSTR_mcat))
            print("Catalyst surface: {:0.2e} [m2/kg_cat]".format(self.CSTR_sbet))
    ...
    def methods():
        ...
        functions = [
            "thermodynamic_consistency_analysis",
            "kinetic_run",
            "map_reaction_rate",
    ...
    def thermodynamic_consistency(self, temperature):
        ...
        for global_reaction in range(self.NGR):
            ...
            print(
                "Model:    DHR={:0.2e} kJ/mol    DSR={:0.2e} kJ/mol/K     DGR={:0.2e} kJ/mol".format(
                    DHR_model[global_reaction],
                    DSR_model[global_reaction],
                    DGR_model[global_reaction],
                )
            )
            print(
                "Database: DHR={:0.2e} kJ/mol    DSR={:0.2e} kJ/mol/K     DGR={:0.2e} kJ/mol".format(
                    DHR_database[global_reaction],
                    DSR_database[global_reaction],
                    DGR_database[global_reaction],
                )
            )
            print("")
            print(
                "Model:    keqH={:0.2e}    keqS={:0.2e}    Keq={:0.2e}".format(
                    keq_H_model[global_reaction],
                    keq_S_model[global_reaction],
                    keq_model[global_reaction],
                )
            )
            print(
                "Database: keqH={:0.2e}    keqS={:0.2e}    Keq={:0.2e}".format(
                    keq_H_database[global_reaction],
                    keq_S_database[global_reaction],
                    keq_database[global_reaction],
                )
            )
            print(
                "----------------------------------------------------------------------------------\n"
    ...
    def kinetic_run(
        ...
        if verbose == 0:
            ...
            print(
                "Temperature = {}K    Pressure = {:.1f}bar".format(
            ...
            str_list = [
                "{}={:.1f}%  ".format(i, j)
                for i, j in zip(sgas, list(np.array(gas_composition) * 100.0))
            ]
        ...
        if initial_conditions is None:  # First surface species is the active site
            ...
            if sum([1 for i in initial_conditions if i < 0]) != 0:
                raise ValueError(
                    "Fatal Error: at least one negative initial surface coverage."
                )
        ...
        if np.sum(y_0[: self.NC_sur]) != 1.0:
            raise ValueError(
                "Fatal Error: the sum of the provided surface coverage is not equal 1."
            )
        if sum(gas_composition) != 1.0:
            raise ValueError(
                "Fatal Error: the sum of the provided molar fractions is not equal to 1."
            )
        for molar_fraction in gas_composition:
            if (molar_fraction < 0.0) or (molar_fraction > 1.0):
                raise ValueError(
                    "Fatal Error: molar fractions must be between 0 and 1."
                )
        if temperature < 0.0:
        ...
        t0 = time.time()
        results = self.__ode_solver_solve_ivp(
            y_0,
            self.reactor.ode,
        ...
        final_y = results.y[:, -1]
        final_ddt = self.reactor.ode(results.t[-1], final_y, *ode_params)
        ...
        for reaction in range(self.NGR):
            try:
                x = self.species_gas.index(self.gr_string[reaction].split()[-3] + "(g)")
            except:
                x = 0
            if self.reactor_model == "dynamic":
        ...
        masi = {
            self.species_sur[np.argmax(final_y[: self.NC_sur - 1])]: max(
                final_y[: self.NC_sur - 1]
            )
        }
        ...
        if verbose == 0:
            print("")
            print(
                "{} Reaction Rate: {:0.2e} 1/s".format(
                    self.target_label, final_r[self.target]
                )
            )
            print("{} Selectivity: {:.2f}%".format(self.target_label, s_target * 100.0))
            print(
                "Most Abundant Surface Intermediate (MASI): {} Coverage: {:.2f}% ".format(
    ...
    def apparent_activation_energy(
        ...
        """
        print(
            "{}: Apparent activation energy for {} reaction".format(
                self.name, global_reaction_label
            )
        )
        print("")
        print(
            "Temperature range: {}-{}K    Pressure = {:.1f}bar".format(
                temp_range[0], temp_range[1] - temp_range[2], pressure / 1e5
            )
        )
        sgas = []
        for i in self.species_gas:
        ...
        print(gas_string)
        print("")
        if (global_reaction_label != self.target_label) and (
            global_reaction_label not in self.by_products_label
        ):
        ...
        for i in range(len(temperature_vector)):
            ...
            if self.reactor_model == "differential":
                ...
                r_ea[i] = self.reactor.reaction_rate(
                    pressure * gas_composition[0],
                    pressure * run["y_out"]["i"],  # TO SOLVE
                    temperature_vector[i],
                )
            print(
                "Temperature = {}K    CPU Time: {:.2f}s".format(
                    temperature_vector[i], time.time() - t0
                )
            )
        ...
        plt.plot(
            1000 / np.array(temperature_vector),
        ...
        plt.title(
            "{}: {} Apparent Activation Energy".format(self.name, global_reaction_label)
        )
        plt.text(
            0.65,
            0.75,
            "$E_{{app}}$={:.0f} kJ/mol\n$R^2$={:.2f}".format(eapp, r_squared),
            transform=fig_ea.transFigure,
            bbox=dict(facecolor="white", alpha=1.0),
        )
        plt.savefig(
            "{}_Eapp_{}_{}{}K_{}bar.png".format(
                self.name,
                global_reaction_label,
                temp_range[0],
                temp_range[1],
    ...
    def apparent_activation_energy_local(
        self,
        temperature,
        pressure,
        gas_composition,
    ...
    def apparent_reaction_order(
        ...
        print(
            "{}: {} Apparent reaction order for {} reaction".format(
                self.name, species_label, global_reaction_label
            )
        )
        print("")
        print(
        ...
        for i in range(n_runs):
            ...
            r_napp[i] = list(run["r"].values())[self.grl[global_reaction_label]]
            print(
                "y_{} = {:.2f}    CPU Time: {:.2f}s".format(
                    species_label, composition_matrix[i, index], time.time() - t0
                )
            )
        ...
        plt.plot(
            np.log(pressure * composition_matrix[:, index]),
        ...
        plt.ylabel("ln($r_{{{}}}$) / $s^{{-1}}$".format(global_reaction_label))
        plt.title(
            "{}: {} apparent reaction order for {}".format(
                self.name, species_label, global_reaction_label
            )
        )
        plt.text(
    ...
    def degree_of_rate_control(
        ...
        if verbose == 0:
            if switch_ts_int == 0:
                print(
                    "{}: DRC analysis for elementary reaction R{} wrt {} reaction".format(
                        self.name, index + 1, global_reaction_label
                    )
                )
            else:
                print(
                    "{}: DRC and DSC for intermediate {} wrt {} reaction".format(
                        self.name, self.species_sur[index], global_reaction_label
                    )
                )
            print(
                "Temperature = {}K    Pressure = {:.1f}bar".format(
        ...
        if switch_ts_int == 0:  # Transition state
            if self.g_ts[index] != 0.0:  # Originally activated reaction
                ...
                for i in range(2):
                    ...
                    if mk_object.dg_reaction[index] < 0.0:
                        ...
                        mk_object.dg_barrier[index] = (
                            mk_object.dg_reaction[index] + dg * i
                        )
                    run = mk_object.kinetic_run(
                        temperature,
                        pressure,
            ...
            for i in range(2):
                ...
                for j in range(mk_object.NR):
                    ...
                    gis = sum(
                        [
                            mk_object.g_species[k] * mk_object.v_matrix[k, j] * (-1)
                            for k in ind
                        ]
                    )
                ...
                if mk_object.reactor_model == "differential":
                    ...
                    r_tot = list(run["r"].values())
                    r_tot = [
                        r_tot[i] for i in range(self.NR) if i in list(self.grl.values())
                    ]
                    s[i] = r[i] / sum(r_tot)
                else:  # dynamic CSTR
                    r[i] = run["R_" + global_reaction_label]
        ...
        if type(ts_int_label) == list:  # multiple species/ts
            ...
            for i in range(len(temp_vector)):
                for j in range(len(ts_int_label)):
                    drc_array[i, j], dsc_array[i, j] = self.degree_of_rate_control(
                        temp_vector[i],
                        pressure,
                        gas_composition,
                        global_reaction_label,
            ...
            drsc_temp = np.concatenate(
                (np.array([temp_vector]).T, drc_array, dsc_array), axis=1
            )
            col = ["T[K]"]
            ...
            df_drsc_temp = pd.DataFrame(np.round(drsc_temp, decimals=2), columns=col)
            ...
            for i in range(len(temp_vector)):
                drc_array[i], dsc_array[i] = self.degree_of_rate_control(
                    temp_vector[i],
                    pressure,
                    gas_composition,
                    global_reaction_label,
            ...
            drsc_temp = np.concatenate(
                (
                    np.array([temp_vector]).T,
                    np.array([drc_array]).T,
                    np.array([dsc_array]).T,
                ),
                axis=1,
            )
            df_drsc_temp = pd.DataFrame(
                np.round(drsc_temp, decimals=2), columns=["T[K]", "DRC", "DSC"]
            )
    ...
    def drc_full(
        ...
        """
        print(
            "{}: Full DRC and DSC analysis wrt {} global reaction".format(
                self.name, global_reaction_label
            )
        )
        print(
        ...
        for reaction in range(self.NR):
            ...
            drc_ts[reaction], dsc_ts[reaction] = self.degree_of_rate_control(
                temperature,
                pressure,
                gas_composition,
                global_reaction_label,
        ...
        for species in range(self.NC_sur):
            ...
            drc_int[species], dsc_int[species] = self.degree_of_rate_control(
                temperature,
                pressure,
                gas_composition,
                global_reaction_label,
        ...
        drsc_ts = np.concatenate((np.array([drc_ts]).T, np.array([dsc_ts]).T), axis=1)
        df_drsc_ts = pd.DataFrame(
            np.round(drsc_ts, decimals=2), index=r, columns=["DRC", "DSC"]
        )
        df_drsc_ts.to_csv(
            "X_{}_{}_{}_ts.csv".format(
                global_reaction_label, int(temperature), int(pressure / 1e5)
            )
        )
        ...
        df_drsc_ts = df_drsc_ts.style.applymap(
            style_significant, props="color:red;"
        ).applymap(lambda v: "opacity: 50%;" if abs(v) < 0.01 else None)
        df_drsc_ts.format({"DRC": "{:,.2f}".format, "DSC": "{:,.2f}".format})
        drsc_int = np.concatenate(
            (np.array([drc_int]).T, np.array([dsc_int]).T), axis=1
        )
        df_drsc_int = pd.DataFrame(
            np.round(drsc_int, decimals=2),
            index=self.species_sur,
            columns=["DRC", "DSC"],
        )
        df_drsc_int.to_csv(
            "X_{}_{}_{}_int.csv".format(
                global_reaction_label, int(temperature), int(pressure / 1e5)
            )
        )
        df_drsc_int = df_drsc_int.style.applymap(
            style_significant, props="color:red;"
        ).applymap(lambda v: "opacity: 50%;" if abs(v) < 0.01 else None)
    ...
    def reversibility(
        self, temperature, pressure, gas_composition, initial_conditions=None
    ):
        """
        ...
        df_reversibility = pd.DataFrame(
            np.round(np.array([reversibility]).T, decimals=2),
            index=r,
        ...
        df_reversibility.to_csv(
            "Z_{}_{}_{}.csv".format(self.name, int(temperature), int(pressure / 1e5))
        )
