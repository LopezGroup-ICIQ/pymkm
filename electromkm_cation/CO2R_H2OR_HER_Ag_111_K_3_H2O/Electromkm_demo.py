...
potential_dl=float(lines[4].split()[1])
cation_ener_error=float(lines[5].split()[1])
...
print(array)
exp = model.kinetic_run(potential, pH, cation_conc, potential_dl,cation_ener_error, gas_composition = array , jac = True)
...
with open('output', 'w') as outfile:
    ...
    for label in model.by_products_label:
        if 'HER' in label or 'H2OR' in label:
            #print(label,'this')
            outfile.write('Current Density (mA/cm2) of {} {:.4e}\n'.format(label,-exp[str('j_'+label)]/20))
        else:
            outfile.write('Current Density (mA/cm2) of {} {:.4e}\n'.format(label,-exp[str('j_'+label)]/10))
