...
def get_NC(species_sur: list, species_gas: list, species_acid: list, species_base: list, species_cation: list, species_tot: list):
    """
    ...
    Returns:
         (int, int, int)
    """
    return len(species_sur), len(species_gas), len(species_acid), len(species_base), len(species_cation), len(species_tot)
    ...
    for line in lines[NGR + 3:]:
        for element in line.split():
            if (element == '+') or (element == '->'):
                pass
            elif (element[0] in {'2', '3', '4','5','6','7','8'}):
                element = element[1:]
            ...
            for species in range(NC_tot):
                if species_label[species] in line:
                    ...
                    if species_index < arrow_index:
                        ...
                        v_matrix[species, reaction] = 2
                elif '8' + species_label[species] in line:
                    species_index = line.index('8' + species_label[species])
                    if species_index < arrow_index:
                        v_matrix[species, reaction] = -8
                    else:
                        v_matrix[species, reaction] = 8
...
def classify_species(species_label):
    ...
    for element in species_label:  # Classification of species (sur/gas)
        if '(g)' in element:
            ...
            cation_switch = 1
            species_cation_label.append(element)
        else:
            species_sur_label.append(element)
    species_sur_label = natsorted(species_sur_label)
    # if CHE_switch == 1  and CHE_switch_base==1 and cation_switch==1:
    #     species_tot = species_sur_label + ["H(e)"] + ["H2O(e)"] + species_cation_label + species_gas_label 
    # elif CHE_switch == 1 and cation_switch==1:
    #     species_tot = species_sur_label + ["H(e)"] + species_cation_label + species_gas_label 
    # elif CHE_switch == 1:
    #     species_tot = species_sur_label + ["H(e)"] + species_gas_label
    # else:
    #     species_tot = species_sur_label + species_gas_label
    species_tot = species_sur_label + species_acid_label + species_base_label + species_cation_label + species_gas_label
    return species_sur_label, species_gas_label, species_acid_label, species_base_label, species_cation_label, species_tot 
...
def ads_mass(v_matrix, reaction_type, NC_sur, NC_acid, NC_base , NC_cation, masses):
    """
    ...
    """
    NR = v_matrix.shape[1]
    NC_gas = v_matrix.shape[0] - NC_sur - NC_cation - NC_acid - NC_base
    m = [0] * NR   
    for i in range(NR):
